...
@RequiredArgsConstructor // Lombok создаст конструктор с обязательными полями
public class SecurityConfig {

    // ИСПРАВЛЕНО: Внедряем новый репозиторий через Lombok @RequiredArgsConstructor
    ...
    public SecurityFilterChain securityFilterChain(HttpSecurity http) throws Exception {
        http
                // 1. ИСПРАВЛЕНИЕ ДЛЯ ПЕЧАТИ (X-Frame-Options)
                ...
                // 2. ИСПРАВЛЕНО: Игнорируем CSRF для API и WebSocket (для Android)
                .csrf(csrf -> csrf.ignoringRequestMatchers("/ws-sellion/**", "/api/**"))

                // 3. ИСПРАВЛЕНО: Добавляем фильтр API Key перед стандартным логином
                .addFilterBefore(apiKeyAuthFilter(), UsernamePasswordAuthenticationFilter.class)

                .authorizeHttpRequests(auth -> auth
                        // API теперь разрешено всем (фильтр выше их аутентифицирует по ключу)
                        .requestMatchers("/api/**").permitAll()

                        // Офисная админка (требует логин/пароль/сессию)
    ...
    public CorsConfigurationSource corsConfigurationSource() {
        // ... (Ваш существующий код CorsConfigurationSource остается без изменений) ...
        CorsConfiguration configuration = new CorsConfiguration();
        configuration.setAllowedOriginPatterns(List.of("*"));
        configuration.setAllowedMethods(List.of("GET", "POST", "PUT", "DELETE", "OPTIONS"));
        configuration.setAllowedHeaders(List.of("Authorization", "Cache-Control", "Content-Type", "X-Requested-With", "X-API-Key")); // Добавлен X-API-Key
